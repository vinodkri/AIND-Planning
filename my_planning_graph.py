...
class PlanningGraph():
    ...
    def create_graph(self):
        ...
        for literal in self.fs.pos:
            n = PgNode_s(literal, True)
            self.s_levels[level].add(n)
            #self.s_levels[level].add(PgNode_s(literal, True))
            level_dict[literal] = n
        for literal in self.fs.neg:
            n = PgNode_s(literal, False)
            self.s_levels[level].add(n)
            #self.s_levels[level].add(PgNode_s(literal, False))
            level_dict[literal] = n
        # no mutexes at the first level
        #print(level_dict)
        self.s_levels_dict.append(level_dict)
    ...
    def add_action_level(self, level):
        ...
        #print("======================= Add Action Level: {} =========================".format(level))

        # subset_of_valid_actions - holds list of action that has satisfied all prerequisite literals for that level

        subset_of_valid_actions = []
        for action in self.all_actions:
            # for each action create new action node
            action_node = PgNode_a(action)
            # Check if all the precondition/prerequisite are met
            if action_node.prenodes.issubset(self.s_levels[level]):
                # append to all valid action list
                subset_of_valid_actions.append(action_node)
                ...
                #print("\ns_level[{}]\n----------------".format(level))
                #print(state_node.show())
                #print("valid action\n----------------\n", action_node.parents)
        self.a_levels.append(subset_of_valid_actions)
        #print("======================= End of Add Action Level: {} ================================".format(level))
    ...
    def add_literal_level(self, level):
        ...
        for action_node in self.a_levels[level-1]:
            for effect_nodes in action_node.effnodes:
                next_state.add(effect_nodes)
                action_node.children.add(effect_nodes)
                effect_nodes.parents.add(action_node)

        d = dict([ (s.symbol, s) for s in next_state ])
        #print (d)

        self.s_levels_dict.append(d)
        self.s_levels.append(next_state)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """
        for n in node_a1.prenodes:
            if n in node_a2.effnodes:
                return True

        for n in node_a2.prenodes:
            if n in node_a1.effnodes:
                return True
        return False
        """
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        if node_s1.is_pos is not node_s2.is_pos:
            return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        """
        level_sum = 0
        # walk through all the goals in the final state
        for goal in self.problem.goal:
            # go up each level
            for level in range(len(self.s_levels)):
                # check if goal state present at each level.
                if goal in self.s_levels_dict[level]:
                    # return goal found !
                    #print("goal found")
        ...
        #print("================================================================================")
        #print("================================================================================")
        #print("Goals to explore: ", len(self.problem.goal))
        """
        for goal in self.problem.goal:
            #print("Goal: {}".format(goal))
            one_goal_found = False
            lev = 0
            for level in range(len(self.s_levels)):
                #print("    ", "Level: ", level)
                for state in self.s_levels[level]:
                    #print("         ", "expr({})".format(goal), "expr({})".format(state.symbol)),
                    #if "expr({})".format(goal) == "expr({})".format(state.symbol):
                    if goal == state.symbol:
                        lev = lev + 1
                        one_goal_found = True
                        #print(goal, state.symbol, "expr({})".format(state.symbol))
                        #print("Matched goal and symbol: ", level_sum)
                        #atleast one goal is found.
                        #print ("************* skip to next goal ************** ")
                        break

                if one_goal_found:
                    level_sum = lev + level_sum
                    #print("********** second break : Goal found! go to next goal *********** ")
                    break
