...
import logging
logger = logging.getLogger('run search')
...
def run_search(problem, search_function, parameter=None):
    ...
    node = None
    start = timer()
    logger.debug("<function_name:{}> Problem: {}".format(run_search.__name__, problem))
    ...
    if node != None:
        ...
        logger.error("<function_name:{}> Node not returned from search function".format(run_search.__name__))
    print()
...
def main(p_choices, s_choices):
    ...
    logger.debug("total problems: {}".format(len(problems)))
    for pname, p in problems:
        logger.debug("pname: {} p: {}".format(pname, p))
        logger.debug("total searches: {}".format(len(searches)))
        for sname, s, h in searches:
            hstring = h if not h else " with {}".format(h)
            print("\nSolving {} using {}{}...".format(pname, sname, hstring))

            _p = p()
            logger.debug("Func {}: problem: {} search: {}".format(main.__name__, _p, s))
